...
import com.apex.ecommerce.model.CartItem;
...
import com.apex.ecommerce.repositories.CartItemRepo;
...
public class CartServiceImpl implements CartService {
    ...
    public CartDTO addProductToCart(Long productId, Integer quantity) {
        ...
        Stream<ProductReqDTO> productStream = cartItems.stream().map(item -> {
            ProductReqDTO map = modelMapper.map(item.getProduct(), ProductReqDTO.class);
    ...
    public List<CartDTO> getAllCarts() {
        ...
        List<CartDTO> cartDTOs = carts.stream().map(cart -> {
            ...
            List<ProductReqDTO> products = cart.getCartItems().stream().map(cartItem -> {
                ProductReqDTO productDTO = modelMapper.map(cartItem.getProduct(), ProductReqDTO.class);
    ...
    public CartDTO getCart(String emailId, Long cartId) {
        ...
        List<ProductReqDTO> products = cart.getCartItems().stream()
                .map(p -> modelMapper.map(p.getProduct(), ProductReqDTO.class))
                .toList();
        cartDTO.setProducts(products);
        return cartDTO;
    }
    ...
    public CartDTO updateProductQuantityInCart(Long productId, Integer quantity) {
        ...
        Stream<ProductReqDTO> productStream = cartItems.stream().map(item -> {
            ProductReqDTO prd = modelMapper.map(item.getProduct(), ProductReqDTO.class);
