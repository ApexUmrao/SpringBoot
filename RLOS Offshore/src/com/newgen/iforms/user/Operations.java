...
public class Operations implements IFormServerEventHandler {
	...
	public String executeServerEvent(IFormReference ifr, String controlId, String eventType, String param) {
		...
		try {
			...
			if ("onClick".equals(eventType)) {
				...
				if(controlId.equalsIgnoreCase("button194"))
				{
					 ...
					 String status="false~Error in fetching data from middleware";
						...
						if (status.split("~")[0].equals("True")) {
							...
							ifr.setValue("IS_LOAN_BOOKED", "Y");
							ifr.setValue("loan_book_date_time", LOS_CommonMethods.getCurrentDateTimeiForm());
							LOS_EG.mLogger.info("Before Calling Send SMS for "+ifr.getObjGeneralData().getM_strProcessInstanceId()+" $$$ Event Loan Booking at LPU Checker");
							if("Y".equalsIgnoreCase(param.split("~")[6]))
							{
								String pType = (String)ifr.getValue("Product_Type");
								//SMS disabled by aarish
//								if(pType.equalsIgnoreCase("CC")) {
//									 String	status1 = Integration_Handler.Send_SMS(ifr,"FinalApprove");
//									 LOS_EG.mLogger.info("Send SMS"+ status1);	
//								}
				...
				/*
				 * else if (controlId.equalsIgnoreCase("button192")) { String
				 * status="false~Error in fetching data from middleware"; try {
				 * status=Integration_Handler.collateralRes(ifr); if
				 * (status.split("~")[0].equals("true")) { ifr.setStyle("button192", "disable",
				 * "true"); ifr.setValue("ReserveCollateral_Status", "Y"); } else {
				 * ifr.setStyle("button192", "disable", "false"); } } catch (Exception e) {
				 * ifr.setStyle("button192", "disable", "false"); } return status; }
				 */
