...
public class RCCU_Risk_Limit implements IFormServerEventHandler {
	...
	public String executeServerEvent(IFormReference ifr, String controlId, String eventType, String param) {
		...
		try {
			...
			if ("insertToDB".equals(eventType)) {
				/*
				 * if ("createHistory".equals(controlId)) { LOS_EG.mLogger.info("controlId : " +
				 * controlId); String query = LOS_CommonMethods.getDataInDecisionHistory(ifr,
				 * param); return query; }
				 */
			}
			...
			if ("onDone".equals(eventType)) {
				...
				if ("Historyset".equals(controlId)) {
					...
					if(decision.contains("Approve"))
					{
						...
						if(ifr.getActivityName().equalsIgnoreCase("RCCU_RiskLimit_Authorizer"))
						{
							...
							if (pType.equalsIgnoreCase("CC")) 
							{
								...
								//SMS removed by aarish
//								LOS_EG.mLogger.info("Before Calling Send SMS for "+ifr.getObjGeneralData().getM_strProcessInstanceId()+" $$$ Event Final Approve at RCCU");
//								String status1=Integration_Handler.Send_SMS(ifr,"FinalApprove");
//								LOS_EG.mLogger.info("[status] : " + status1);
							}	
						}	
					}
