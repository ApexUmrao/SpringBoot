...
public class LOS_RCR_Credit_TL implements IFormServerEventHandler {
		...
		try {
			...
			if ("onDone".equals(eventType)) {
				...
				if ("Historyset".equals(controlId)) {
					...
					LOS_EG.mLogger.info("Before Calling Send SMS for "+ifr.getObjGeneralData().getM_strProcessInstanceId()+" $$$ Event Initial Approve at RCR Credit TL");
					 String	status1 = Integration_Handler.Send_SMS(ifr,"InitialApprove");
